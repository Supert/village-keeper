...
public class MonsterScript : MonoBehaviour
{
    ...
    public Vector2 Size
    {
        get
        {
            return (Vector2)_sprite.bounds.size;
        }
    }
    ...
    public bool CheckHitByPosition(Vector3 projectilePosition)
    {
        ...
        {
            if (Mathf.Abs(projectilePosition.z - transform.localPosition.y) <= _sprite.bounds.extents.y)
            {
                TakeDamage();
                return true;
            }
    ...
    {
        if (_sprite.color != targetColor)
            _sprite.color = Vector4.MoveTowards(_sprite.color, targetColor, Time.deltaTime / animationDuration);
    }
    ...
    void Start()
    {
        ...
        ls *= (CoreScript.Instance.BuildingsArea.CellWorldSize.y * 2) / (_sprite.bounds.size.y);
